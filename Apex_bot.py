...
@client.event
async def on_ready():
    await client.tree.sync(guild=guild_id)  # Sync commands with the Discord guild
...
async def اشتم(interaction: discord.Interaction, member: discord.Member ):
    owner_state = 0
    if interaction.user.id in owner_id : owner_state = 1
...
async def  بوس (interaction: discord.Interaction, member: discord.Member):
...
async def os(interaction: discord.Interaction ):
    x= interaction.user.id 
    if x in owner_id:
        await interaction.response.send_message("sa7y ya 3mna ♥")
    else : await interaction.response.send_message("Not Allowed To Use This Command 😔")
...
async def map_rotation(interaction: discord.Interaction , mode : app_commands.Choice[str] ):
    embeds = [] 
            ...
            if response.status == 200:
                ...
                if mode.value == 'all':
                    for index,x in enumerate(data):
                        title = f"**Mode**: {x[:200]}"  # Truncate to fit under the 256 char limit
                        embed = discord.Embed(title=title, description="Current and Next Map Rotation", color=colors[index])
                        # Current Map Data
                        try:
                            current_map = data[x]['current']['map']
                            current_asset = data[x]['current']['asset']
                            remaining_timer = data[x]['current']['remainingTimer']
                            try :current_event_name = data[x]['current']['eventName'] 
                        ...
                        try:
                            ...
                            try :next_event_name = data[x]['next']['eventName']
                            ...
                            embed.add_field(name="**❌ Error:**", value="Unable to fetch next map data.", inline=False)
                ...
                # Respond once with the first embed
                await interaction.response.send_message(embed=embeds[0])

                # Follow up with the remaining embeds
                for embed in embeds[1:]:
                    await interaction.followup.send(embed=embed)  # Use followup to send additional embeds
...
async def server_status(interaction: discord.Interaction):
    await interaction.response.defer()  # Defer response to allow multiple follow-ups
        ...
        async with session.get(server_status_url, headers=HEADERS) as response:
            if response.status == 200:
                if response.content_type == 'text/plain':
                    ...
                    # Dictionary to store formatted messages for each server region
                    server_messages = {}
...
async def get_player_statistics(interaction: discord.Interaction, player_name: str, platform: app_commands.Choice[str]):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(player_statistics_url, headers=HEADERS) as response:
            if response.status == 200 and response.content_type == "application/json":
                ...
                try :headshots = data["total"].get("headshots", {}).get("value",  "Not Found")
                except : headshots = "Not Found"
                try :season_kills = data["total"]["kills"]["value"]
                except : season_kills = "Not Found"
                try : season_damage = data["total"]["damage"]["value"]
                except : season_damage = "Not Found"
                try :total_wins = data["total"]["career_wins"]["value"]
                except : total_wins = "Not Found"
                try :total_kills = data["total"]["career_kills"]["value"]
                except : total_kills = "Not Found"
                try :matches_played = data["total"].get("matches", "Not Found")
                except : matches_played = "Not Found"
